...
from src.generator.fraud_injector import inject_account_takeover


def _sample_data():
    customers = pd.DataFrame(
        {
            "customer_id": ["C0000001", "C0000002"],
            "province": ["Gauteng", "Western Cape"],
            "avg_transaction": [500.0, 1000.0],
            "std_transaction": [100.0, 250.0],
            "usual_start_hour": [7, 8],
            "usual_end_hour": [21, 22],
        }
    )

    accounts = pd.DataFrame(
        {
            "account_id": ["A00000001", "A00000002"],
            "customer_id": ["C0000001", "C0000002"],
            "account_type": ["CHEQUE", "CHEQUE"],
            "opening_balance": [50000.0, 75000.0],
    ...
    transactions = pd.DataFrame(
        {
            ...
            "is_international": [False, False],
            "direction": ["DEBIT", "DEBIT"],
            "balance_before": [50000.0, 75000.0],
            "balance_after": [49500.0, 74000.0],
        }
    )
...
def test_legitimate_transactions_remain_legitimate():
    ...
    original_ids = set(transactions["transaction_id"])

    original_rows = result[
        result["transaction_id"].isin(original_ids)
    ]

    assert (original_rows["is_fraud"] == 0).all()
    assert original_rows["fraud_type"].isna().all()
    assert original_rows["fraud_scenario_id"].isna().all()
...
def test_fraud_has_correct_ground_truth():
    ...
    fraud = result[result["is_fraud"] == 1]

    assert not fraud.empty
    assert (fraud["fraud_type"] == "ACCOUNT_TAKEOVER").all()
    assert fraud["fraud_scenario_id"].notna().all()


def test_fraud_uses_new_device():
    customers, accounts, transactions = _sample_data()

    original_devices = set(transactions["device_id"])
    ...
    fraud_devices = set(
        result.loc[result["is_fraud"] == 1, "device_id"]
    )

    assert fraud_devices.isdisjoint(original_devices)
...
def test_foreign_keys_remain_valid():
    ...
    assert set(result["customer_id"]).issubset(
        set(customers["customer_id"])
    )

    assert set(result["account_id"]).issubset(
...
def test_fraud_amounts_are_positive():
    ...
    assert (fraud["balance_after"] >= 0).all()
